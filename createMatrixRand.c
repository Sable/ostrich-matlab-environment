...
void createMatrixRand(double* m, int dim0, int dim1){
    int col, row;
    
    for(row=0; row<dim0; row++)
        for(col=0; col<dim1; col++){
            m[col*dim0+row] = (double)common_rand();
        }
}

void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[]){
    if(nrhs != 2){
        mexErrMsgIdAndTxt("createMatrixRand", "Two inputs required");
    }
